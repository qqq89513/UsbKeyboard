...
/*
	...
	Circuit in folder /UsbKeyboard/examples/UsbKeyboardNkeyDown/circuit/
*/
...
void loop(){
  ...
  for(byte i=BUTTON_PINstart; i<=BUTTON_PINend; i++)//check each pin state
  {
    ...
    {
      UsbKeyboard.keyDown(keyMap[i-BUTTON_PINstart]);
    ...
    {
      UsbKeyboard.keyUp(keyMap[i-BUTTON_PINstart]);
